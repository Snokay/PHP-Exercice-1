...
if ($versed > $price){
    $remainder = abs($price - $versed);
    $amout = abs($price - $versed);
...
//If the person has not given enough to show the value the person still needs to pay.
else if ($versed < $price){
    $remainder = abs($versed - $price);

    echo nl2br("\n Montant à payer : $price €");
    echo nl2br("\n Montant versé : $versed €");
    echo nl2br("\n *********************************************************");
    echo nl2br("\n le client dois encore payer : $remainder €");
}

//If the price is correct display the sum is round.
else if ($versed = $price){
    echo nl2br("\n Montant à payer : $price €");
    echo nl2br("\n Montant versé : $versed €");
    echo nl2br("\n *********************************************************");
    echo nl2br("\n le client a payer la somme pile");
}
